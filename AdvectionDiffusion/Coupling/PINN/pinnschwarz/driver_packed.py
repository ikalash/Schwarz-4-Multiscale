...
class Driver:
    ...
    def initialize_pde(self, label_dic):
        ...
        domain_size = 1 / (
            label_dic["n_subdomains"] * (1 - label_dic["percent_overlap"]) + label_dic["percent_overlap"]
        )
        ...
        X_b_om = [
            [tf.constant(np.tile([i], (2 ** int(self.hyper["PDE"]["b_power"]), 1)), dtype=DTYPE) for i in sub[j]]
    ...
    def train(self, x_true, u_true, label_dic, model_om, X_r_om, X_b_om, sub, pde1):
        ...
        figdir = os.path.join(
            ...
            + label_dic["BC_label"]
            + label_dic["FOM_label"]
            + "_Pe_{:d}_nSub_{:d}_over{:f}".format(
                int(beta / (1 / label_dic["Pe"])), label_dic["n_subdomains"], label_dic["percent_overlap"]
            ),
        )
