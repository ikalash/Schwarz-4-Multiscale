        ...
        for i, model in enumerate(self.model_i):
            ...
            if not model:
                ...
                if boundary_pt in Neum_pts:
                    ...
                    n1 = (-1) ** (boundary_pt + 1)
                    n2 = (-1) ** (boundary_pt)
                    phi_i += (1 - self.a) * tf.reduce_mean(
                        tf.square((L * u_pred1 + n1 * du_pred1) - (L * u_pred2 + n2 * du_pred2))
                    )
...
def phi_transform(bounds, x):
    ...
    a4 = (
        (gi**3 - 3 * gip1 * gi**2 + 3 * gi * gip1**2 - 4 - gip1**3)
        * gi
        * gip1
...
def psi_transform(bounds, x):
    ...
    b3 = (
        ...
        * (
        ...
        / (gi**3 - gip1 * gi**2 - gi * gip1**2 + gip1**3)
    )
