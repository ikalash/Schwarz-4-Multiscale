function [output] = TopLevelLagrange_Accel(settings)
    ...
    disp('TopLevelLagrange_Accel method');
    ...
    % external loads (not contact
    R = zeros(num_nodes_total, 1);
    ...
    for idt = 1:(nt-1)
      ...
      A_mat(1:num_nodes_total, 1:num_nodes_total) = b2*M + b1 * C + b0 * K;
      A_mat(1:num_nodes_total, end) = G';
      A_mat(end, 1:num_nodes_total) = G;
      A_mat(end, end) = -G * (q0);

      b(1:num_nodes_total) = R - f_int - (M * q2 + C * q1 + K * q0);
      b(end) = 0;
      ...
      u_new = q0 + b0 * delta_d2udt2_new;
      dudt_new = q1 + b1 * delta_d2udt2_new;
      d2udt2_new = q2 + b2 * delta_d2udt2_new;

      % now move to next time step
      u = 0;
      dudt = dudt_new;
      d2udt2 = d2udt2_new;
