...
%       if abs(domain_1_pt - domain_2_pt) < 1.0e-4
%           contact(idt) = true;
%         else
          contact(idt) = true;
%         end  
...
%         u(start_idx:end_idx) = XV{idomain}(:, idt);
      ...
      % midpoint rule
%       A_mat1 = K_aug + 2 / dt * C_aug + 4 / dt^2 * M_aug;  
...
%             b = f_ext - f_int - ...
%                   M_aug * (u / (beta * dt^2) - dudt / (beta * dt) -  (1 / (2*beta) - 1) * d2udt2) - ...
%                   C_aug * (gamma * u / (beta * dt) + dudt* (1 - gamma /
%                   beta) + dt*d2udt2 * (1 - gamma / (2*beta)) );

%           b = f_ext - f_int + ...
%           M_aug * (u / (beta * dt^2) + dudt / (beta * dt) +  (1 / (2*beta) - 1) * d2udt2) + ...
%           C_aug * (gamma * u / (beta * dt) - dudt* (1 - gamma / beta) - dt*d2udt2 * (1 - gamma / (2*beta)) );
    
%             b = f_ext - f_int - M_aug * d2udt2 - C_aug * dudt ;

%             u_new = zeros(num_nodes_total+1, 1);

%             for idomain = 1:num_domains
% %             A_ii = A_domains{idomain};
%               A_ii = A_mat((left_domain_indices(idomain):right_domain_indices(idomain)),(left_domain_indices(idomain):right_domain_indices(idomain)));
...
%             end
            ...
            it_count = it_count + 1;
          end % it_count < max_lagrange_iterations
      ...
      else % augmented_lagrange_mode == false

        if contact(idt)
          A_mat(num_nodes_total +1, contact_node_pair(1)) = 1 * relaxation_factor;
          A_mat(num_nodes_total +1, contact_node_pair(2)) = -1 * relaxation_factor;
          A_mat(contact_node_pair(1), num_nodes_total + 1) = 1 * relaxation_factor;
          A_mat(contact_node_pair(2), num_nodes_total + 1) = -1 * relaxation_factor;
          A_mat(num_nodes_total + 1, num_nodes_total + 1) =  0 * relaxation_factor;
          b(num_nodes_total + 1,1) = (domain_2_pt - domain_1_pt) * relaxation_factor;

          % solve
          u_new = A_mat \ (b);
%           lam = (sparse(G)) \ sparse(r);
%           u_new = u_new1 + lam;
        else % no contact 
...
%         XV{idomain}(:, idt + 1) = delta_u{idomain};
        % midpoint rule
